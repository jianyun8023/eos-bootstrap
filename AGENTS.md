...
- `ansible/playbook.yml` — single playbook, `hosts: localhost`, `become: true`. Role order is significant: `packages → mise → services → network → kernel → logging → power → fan-profile → fingerprint → user → display`.
...
- **Fingerprint PAM is per-service and opt-in.** The role installs native `libfprint` support for configured USB IDs and retains a separate FocalTech shim path. Enroll and verify a finger before setting `fingerprint_pam_services`. Avoid `sudo`, `polkit-1`, and `system-auth` because of CVE-2024-37408; the role blocks them unless `fingerprint_allow_high_risk_pam` is explicitly true. Keep `i3lock` out of PAM too: the upstream dotfiles lock wrapper uses password-only authentication. `fprintd.service` is D-Bus activated and must not be enabled as a normal boot service.
...
- Upstream main at `302e1a6` is merged; NVIDIA S0ix and upstream fingerprint changes are excluded. Preserve this fork's Synaptics/PAM support.
- Retain HP identity, xrdp fixes, WezTerm nightly, desktop runtime dependencies, and `DOTFILES_SOURCE_DIR` support.
- Keep dynamic `include_tasks` tags on both the include and `apply.tags` so selective runs execute inner tasks.
- Lock-on-suspend uses the upstream dotfiles locker/user service. This fork dispatches it before sleep.target and waits for i3lock's input-grab acknowledgement; system-sleep hooks cannot synchronously call frozen user services on systemd 261.
- `power_lid_watchdog_enabled` defaults to false on the HP; enable only for confirmed lid sensor chatter. Power-button policy is adopted separately.
- AUR installation is skipped in Ansible check mode; package-presence probes remain read-only.
