...
- **Fingerprint PAM is per-service and opt-in.** The role installs native `libfprint` support for configured USB IDs and retains a separate FocalTech shim path. Enroll and verify a finger before setting `fingerprint_pam_services`. Avoid `sudo`, `polkit-1`, and `system-auth` because of CVE-2024-37408; the role blocks them unless `fingerprint_allow_high_risk_pam` is explicitly true. `fprintd.service` is D-Bus activated and must not be enabled as a normal boot service.
